...
async function getNextId() {
    const querySnapshot = await getDocs(query(collection(db, 'previsiones'), orderBy('customId', 'desc')));
    let lastId = 0;
...
// Función para verificar si el nombre de la previsión ya existe (case-insensitive)
async function isNombreTaken(nombrePrevision, excludeDocId = null) {
    const q = query(collection(db, 'previsiones'), where('nombrePrevisionLower', '==', nombrePrevision.toLowerCase()));
...
registrarBtn.addEventListener('click', async () => {
    const nombrePrevision = document.getElementById('nombre-prevision').value.trim();
    const userName = currentUser ? currentUser.nombreCompleto : 'Usuario';
    const fechaCreacion = getLocalDate();

    if (!nombrePrevision) {
        showMessage('Por favor, ingrese el nombre de la previsión.', 'error');
        return;
    }

    const nombreTaken = await isNombreTaken(nombrePrevision);
    if (nombreTaken) {
        showMessage(`La previsión "${nombrePrevision}" ya está registrada.`, 'error');
        return;
    }
    ...
    try {
        ...
        await addDoc(collection(db, 'previsiones'), {
            customId,
            nombrePrevisionDisplay: nombrePrevision,
            nombrePrevisionLower: nombrePrevision.toLowerCase(),
            fechaCreacion,
            usuario: userName,
            createdAt: new Date()
        });
        ...
        showMessage(`Se ha registrado exitosamente la previsión ${nombrePrevision}.`, 'success');
...
async function exportToExcel() {
    ...
    try {
        ...
        const data = filteredRecords.map(record => ({
            ID: record.id,
            'Previsión': record.nombrePrevisionDisplay || '',
...
function applyFilters() {
    filteredRecords = allRecords.filter(record => {
        return Object.keys(activeFilters).every(column => {
            ...
            else if (column === 'nombrePrevision') cellValue = record.nombrePrevisionDisplay || '';
...
async function loadTableData() {
    ...
    try {
        const querySnapshot = await getDocs(query(collection(db, 'previsiones'), orderBy('customId')));
        ...
        querySnapshot.forEach(doc => {
            ...
            const record = {
                ...
                docId: doc.id,
                nombrePrevisionDisplay: data.nombrePrevisionDisplay,
                nombrePrevisionLower: data.nombrePrevisionLower,
        ...
        if (allRecords.length === 0) {
            previsionesTableBody.innerHTML = '<tr><td colspan="5">No hay previsiones registradas.</td></tr>';
            return;
        }
        ...
        previsionesTableBody.innerHTML = `<tr><td colspan="5">Error al cargar datos: ${error.message}</td></tr>`;
    }
}
...
function renderTable() {
    ...
    if (filteredRecords.length === 0) {
        previsionesTableBody.innerHTML = '<tr><td colspan="5">No hay previsiones que coincidan con los filtros.</td></tr>';
        return;
    }
    ...
    pageRecords.forEach(record => {
        ...
        row.innerHTML = `
            ...
            <td>${record.nombrePrevisionDisplay || ''}</td>
            <td>${fechaCreacion}</td>
        ...
        previsionesTableBody.appendChild(row);
    });
...
function openEditModal(record) {
    ...
    document.getElementById('edit-nombre-prevision').value = record.nombrePrevisionDisplay || '';
    editModal.style.display = 'flex';
}

saveEditBtn.addEventListener('click', async () => {
    const nombrePrevision = document.getElementById('edit-nombre-prevision').value.trim();
    ...
    if (nombrePrevision.toLowerCase() !== editingRecord.nombrePrevisionLower) {
        const nombreTaken = await isNombreTaken(nombrePrevision, editingRecord.docId);
        if (nombreTaken) {
            showMessage(`La previsión "${nombrePrevision}" ya está registrada.`, 'error');
            return;
        }
    ...
    try {
        ...
        const previsionDocRef = doc(db, 'previsiones', editingRecord.docId);
        await updateDoc(previsionDocRef, updatedData);

        registerModal.style.display = 'none';
        showMessage(`Se ha actualizado exitosamente la previsión ${nombrePrevision}.`, 'success');
        await loadTableData();
    } catch (error) {
...
async function openDeleteModal(record) {
    ...
    deleteMessage.textContent = `¿Desea eliminar la previsión ${record.nombrePrevisionDisplay}?`;
    ...
    document.getElementById('confirm-delete-btn').onclick = async () => {
        try {
            ...
            await new Promise(resolve => simulateProgress(registerProgress, resolve));
            await deleteDoc(doc(db, 'previsiones', record.docId));

            registerModal.style.display = 'none';
            document.getElementById('delete-modal').style.display = 'none';
            showMessage(`Se ha eliminado la previsión ${record.nombrePrevisionDisplay}.`, 'success');
            await loadTableData();
        } catch (error) {
...
async function initialize() {
    onAuthStateChanged(auth, async (user) => {
        ...
        try {
            ...
            loadingModal.style.display = 'none';
            previsionesTableBody.innerHTML = `<tr><td colspan="5">Error al inicializar: ${error.message}</td></tr>`;
        }
    });
}
