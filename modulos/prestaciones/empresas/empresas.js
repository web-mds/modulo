...
async function getNextId() {
    const querySnapshot = await getDocs(query(collection(db, 'empresas'), orderBy('customId', 'desc')));
    let lastId = 0;
...
// Función para verificar si el RUT de la empresa ya existe
async function isRutTaken(rutEmpresa, excludeDocId = null) {
    const q = query(collection(db, 'empresas'), where('rutEmpresa', '==', rutEmpresa));
...
// Función para verificar si el nombre de la empresa ya existe
async function isNombreTaken(nombreEmpresa, excludeDocId = null) {
    const q = query(collection(db, 'empresas'), where('nombreEmpresa', '==', nombreEmpresa));
    const querySnapshot = await getDocs(q);
    if (excludeDocId) {
...
// Función para crear y mostrar un input flotante
...
// Función para manejar clics en los íconos de filtrado
...
async function loadTableData() {
    ...
    try {
        const querySnapshot = await getDocs(query(collection(db, 'empresas'), orderBy('customId')));
...
saveEditBtn.addEventListener('click', async () => {
    ...
    // Verificar si el nombre ha cambiado y si está tomado por otra empresa
    ...
    // Verificar si el RUT ha cambiado y si está tomado por otra empresa
    ...
    try {
        ...
        const empresaDocRef = doc(db, 'empresas', editingRecord.docId);
...
async function openDeleteModal(record) {
    ...
    document.getElementById('confirm-delete-btn').onclick = async () => {
        try {
            ...
            await new Promise(resolve => simulateProgress(registerProgress, resolve));
            await deleteDoc(doc(db, 'empresas', record.docId));
