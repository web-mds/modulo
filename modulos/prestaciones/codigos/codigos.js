...
const dbCodigos = getFirestore(appCodigos); 
...
function validatePriceFormat(priceStr) {
    const regex = /^[0-9]+(\.[0-9]{3})*$/;
    return regex.test(priceStr.trim());
}
...
registrarNuevoBtn.addEventListener('click', async () => {
    const referencia = nuevoReferencia.value.trim().toUpperCase();
    const detalles = nuevoDetalles.value.trim().toUpperCase();
    const precioNeto = document.getElementById('nuevo-precio-neto').value.trim();
    ...
    if (!validatePriceFormat(precioNeto)) {
        showMessage('El precio neto debe tener el formato correcto (ej. 860.558).', 'error');
        return;
    }
...
registrarExistenteBtn.addEventListener('click', async () => {
    ...
    const precioNeto = document.getElementById('existente-precio-neto').value.trim();
    ...
    try {
        ...
        for (let i = 0; i < totalRows; i += batchSize) {
            ...
            for (let j = i; j < end; j++) {
                ...
                const precioNeto = row['Precio'] ? String(row['Precio']).trim() : '';
                ...
                if (!validatePriceFormat(precioNeto)) {
                    batchErrors.push(`Fila ${j + 1}: El precio neto "${precioNeto}" tiene un formato inválido (ej. 860.558).`);
                    continue;
                }
...
function applyFilters() {
    ...
    filteredRecords = records.filter(record => {
        return Object.keys(activeFilters).every(column => {
            ...
            else if (column === 'precioNeto') cellValue = record.precioNeto || '';
...
function renderTable() {
    ...
    pageRecords.forEach(record => {
        ...
        row.innerHTML = `
            ...
            <td style="width: ${columnWidths[5]}px; min-width: ${columnWidths[5]}px" title="${record.precioNeto || ''}">${record.precioNeto || ''}</td>
...
saveEditBtn.addEventListener('click', async () => {
    ...
    const precioNeto = document.getElementById('edit-precio-neto').value.trim();
