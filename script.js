...
// Configurar la persistencia de la sesión a solo la sesión del navegador
...
onAuthStateChanged(auth, async (user) => {
    if (!user) {
        ...
        try {
            ...
            // Iniciar el temporizador de cierre de sesión (5 horas = 5 * 60 * 60 * 1000 milisegundos)
    ...
    menuItems.forEach(item => {
        item.addEventListener('click', () => {
            ...
            subOptions[section].forEach((option, index) => {
                ...
                const iconClass = isBelowDivider ? 'fas fa-folder submenu-icon' : 'fas fa-list submenu-icon';
                li.innerHTML = `
                    <i class="${iconClass}"></i>
